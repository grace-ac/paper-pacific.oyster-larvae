...
```{r}
PeakAreas <- read.csv("../data/20220611-PeakAreas-peptides.csv")
head(PeakAreas)
```
...
```{r}
head(testResultOneComparison$ComparisonResult) #View results
```
...
```{r}
write.csv(peptideComparisonResults, "../analyses/MS_stats/peptides/20220617-2015Cgseed-peptidecomp.csv", col.names = c("Peptide", "Label", "log2FC", "SE", "Tvalue", "DF", "pvalue"), row.names = F) #write out comparison
```
...
#### Volano
```{r}
groupComparisonPlots(data = peptideComparisonResults, type = "VolcanoPlot") #Volcano plot, alpha = 0.05.
```
Manually move to analyses/MS_stats/peptides (06172022)

Volcano plot shows 2 peptides up-regulated and 1 peptide down-regulated, all the rest are not significant

In the data file, those three peptides are only present in one or the other temperature.  

#### Comparison
```{r}
groupComparisonPlots(data = peptideComparisonResults, type = "ComparisonPlot") #Comparison plot, alpha = 0.05
```
Manually move to analyses/MS_stats/peptides (06172022)
